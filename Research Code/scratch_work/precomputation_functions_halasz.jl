...
function primes_less_than_z(z, prime_set = primes)
    i = 1
    while primes[i] <= z
        i += 1 
    end
...
function all_combination_products_less_than_z(z, n)
    return sort(append!([1], [prod(entry) for entry in combinations(primes_less_than_z(z)) if 0 < prod(entry) <= n]))
end

function end_points_for_intervals(z, n)
    return sort(append!([z], [Int64(floor(n/entry)) for entry in all_combination_products_less_than_z(z, n)]))
end
